...
public class CameraViewField : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        */
        ...
        //直線定位後旋轉
        ViewField_Value = GetComponent<Camera>().fieldOfView/2;

        _lineRenderer[0].transform.rotation = Quaternion.Euler(new Vector3(-ViewField_Value, 0,0));
        _lineRenderer[0].SetPosition(0, transform.position);
        _lineRenderer[0].SetPosition(1, ChangeSphereObj[0].transform.position);

        _lineRenderer[1].transform.rotation = Quaternion.Euler(new Vector3(ViewField_Value, 0, 0));
        _lineRenderer[1].SetPosition(0, transform.position);
